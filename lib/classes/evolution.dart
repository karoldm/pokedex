  ...
  void buildTriggerTarget(List<dynamic> evoDetails) {
    for (Map<String, dynamic> details in evoDetails) {
      ...
      if (details["gender"] != null) {
        triggers
            .add(capitalizeWords(details['gender']['name'], '-', replace: ' '));
      }

      if (details["held_item"] != null) {
        triggers.add(
            "Holding ${capitalizeWords(details['held_item']['name'], '-', replace: ' ')}");
      }

      if (details["item"] != null) {
        triggers.add(
            "${capitalizeWords(details['item']['name'], '-', replace: ' ')}");
      }

      if (details["known_move"] != null) {
        triggers.add(
            "Know ${capitalizeWords(details['known_move']['name'], '-', replace: ' ')} move");
      }

      if (details["known_move_type"] != null) {
        triggers.add(
            "Knows move of type ${capitalizeWords(details['known_move_type']['name'], '-', replace: ' ')}");
      }

      if (details["location"] != null) {
        triggers.add(
            "At ${capitalizeWords(details['location']['name'], '-', replace: ' ')}");
      }
      ...
      if (details["needs_overworld_rain"] != null &&
          details["needs_overworld_rain"] == true) {
        triggers.add("Needs Overworld Rain");
      }

      if (details["party_species"] != null) {
        triggers.add(
            "Party with ${capitalizeWords(details['party_species']['name'], '-', replace: ' ')}");
      }

      if (details["party_type"] != null) {
        triggers.add(
            "Party with ${capitalizeWords(details['party_type']['name'], '-', replace: ' ')}");
      }
